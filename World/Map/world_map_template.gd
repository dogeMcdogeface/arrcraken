...
func update_debug_navigation_visuals():
		...
		$debug/visual_shallow_water.set_polygons(array)

		array = []
		for n in $deep_water.navigation_polygon.get_polygon_count():
			array.append($deep_water.navigation_polygon.get_polygon(n))
		$debug/visual_deep_water.set_polygons(array)

		$debug/visual_shallow_water.set_polygon($shallow_water.navigation_polygon.get_vertices())
		$debug/visual_deep_water.set_polygon($deep_water.navigation_polygon.get_vertices())
...
#TODO clean up
		...
		if old_gateway_polygon.hash() != array.hash():
			...
			var children = $debug/visual_connection_water.get_children()
			for child in children:
				child.queue_free()
			for n in array:
				var line = Line2D.new()
				line.default_color = $debug/visual_connection_water.default_color
				line.width = $debug/visual_connection_water.width
				$debug/visual_connection_water.add_child(line)
				line.add_point(n[0])
				line.add_point(n[1])
