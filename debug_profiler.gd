...
var debug :bool = true
var debug_string = "\
time_scale:       %6.3f 				| max_physics_steps_per_frame: %-10.0f
max_fps:          %4.0f					| physics_ticks_per_second:    %-2.0f
frame_time  (ms): %6.3f (%-5.0f)		| process_time      (ms): %-2.3f (%-5.0f)	
physics_time(ms): %6.3f (%-5.0f)		| physics_frame_time(ms): %-2.3f (%-5.0f)"
